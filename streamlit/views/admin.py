...
from backend.utils.time import now
from backend.utils.validation import is_not_empty, is_valid_email
...
from backend.utils.logout import logout

# =====================================================
# Dashboard
# =====================================================
def dashboard():
    from backend.services.analytics_service import get_kpi_summary
    ...
    with st.container(border=True):
        ...
        total_s = float(kpi['total_duration'])
        hrs = int(total_s // 3600)
        mins = int((total_s % 3600) // 60)
        c3.metric("Audio Duration", f"{hrs} hrs, {mins} mins")
    ...
    with st.container(border=True):
        ...
        # Streamlit Task Pipeline must show "Rework Needed" based on rework state.
        c4.metric("Rework Needed", f"{kpi.get('returned_count', 0)} 🔄")
...
def users():
    ...
    if users_list:
        ...
        for user in users_list:
            ...
            # st.divider() removed to reduce excessive spacing
    ...
    st.write("") # small gap instead of divider
...
def analytics():
    ...
    # TAB 4: Leaderboards (Extracted, no longer here)
    with tab_leaderboards:
        st.info("Leaderboards have moved to the main sidebar.")
...
def curation():
    ...
    st.title("🎙️ Curation")
    st.markdown("Select a language to upload audio and run the curation pipeline.")
    
    if "curation_lang" not in st.session_state:
        st.session_state.curation_lang = "hindi"
        
    st.session_state.curation_lang = st.radio("Select Language", ["hindi", "english", "telugu"], format_func=lambda x: x.capitalize())
    ...
    if st.button("Upload and Process"):
        ...
        with st.spinner("Uploading..."):
            ...
            data = {"language": st.session_state.curation_lang}
            cookies = {"akshara_session": token}
            
            try:
                res = requests.post("http://127.0.0.1:8000/api/curation/upload", files=files, data=data, cookies=cookies)
                res.raise_for_status()
                audio_data = res.json()
                st.session_state.curation_audio_id = audio_data.get("id")
                st.success("Uploaded successfully. Pipeline started!")
                st.rerun()
            except Exception as e:
                st.error(f"Upload failed: {e}")
                return
                
    audio_id = st.session_state.get("curation_audio_id")
    if audio_id:
        st.write("---")
        ...
        try:
            res = requests.get(f"http://127.0.0.1:8000/api/curation/status/{audio_id}", cookies=cookies)
            if res.status_code == 200:
                status_data = res.json()
                status = status_data.get("pipeline_status")
                
                status_placeholder.info(f"Current Status: **{status}**")
                
                if status == "COMPLETED":
                    st.success("Transcript Generated!")
                    if st.button("Submit to Annotation Queue"):
                        sub_res = requests.post(f"http://127.0.0.1:8000/api/curation/{audio_id}/submit", cookies=cookies)
                        if sub_res.status_code == 200:
                            st.success("Submitted successfully!")
                            st.session_state.curation_audio_id = None
                            st.rerun()
                        else:
                            st.error("Submit failed.")
                elif status == "FAILED":
                    st.error(f"Pipeline failed: {status_data.get('error')}")
                elif status == "SUBMITTED":
                    st.success("Already submitted to queue.")
        except Exception as e:
...
def exports():
    st.title("📦 Exports")
    st.info("Download finalized annotation packages for completed tasks.")
    ...
    db = SessionLocal()
    try:
        completed_files = db.query(AudioFile).filter(AudioFile.status == "COMPLETED").all()
        
        if not completed_files:
            st.warning("No completed audio files found.")
            return

        c1, c2, c3, c4, c5 = st.columns([3, 2, 2, 2, 2])
        c1.markdown("**File Name**")
        c2.markdown("**Language**")
        c3.markdown("**Duration**")
        c4.markdown("**Status**")
        c5.markdown("**Action**")
        st.divider()

        from backend.api.routers.annotations import export_annotation_srt

        for audio in completed_files:
            c1, c2, c3, c4, c5 = st.columns([3, 2, 2, 2, 2])
            with c1:
                st.write(audio.original_filename or audio.filename)
            with c2:
                st.write(audio.language.capitalize())
            with c3:
                total_s = float(audio.duration)
                hrs = int(total_s // 3600)
                mins = int((total_s % 3600) // 60)
                st.write(f"{hrs} hrs, {mins} mins")
            with c4:
                st.success("Completed")
            with c5:
                # We use a unique key for each download button
                zip_bytes = None
                
                # To prevent loading all ZIPs into memory on page load,
                # we could just provide a "Prepare Download" button first, 
                # but for simplicity, since it's just Streamlit, we will use a button 
                # to trigger the ZIP creation.
                if st.button("Generate ZIP", key=f"gen_zip_{audio.id}"):
                    try:
                        res = export_annotation_srt(audio_id=audio.id, db=db, current_user=st.session_state.user)
                        zip_bytes = res.body
                    except Exception as e:
                ...
                if zip_bytes:
                    st.download_button(
                        label="Download ZIP",
                        data=zip_bytes,
                        file_name=f"export_{audio.filename}.zip",
                        mime="application/zip",
                        key=f"dl_zip_{audio.id}"
                    )
            st.divider()
...
def settings():
    ...
    st.markdown("Manage your preferences and system configuration.")
    st.divider()
    ...
    with col1:
        ...
        with st.container(border=True):
            ...
            if st.button("🔑 Change Password"):
                st.info("Change password modal goes here.")

        # Task & Annotation
        with st.container(border=True):
            st.subheader("Task & Annotation")
            st.checkbox(
                "Auto-Assign Tasks",
                value=True,
                help="Automatically assign new audio to available annotators"
            )
        ...
        with st.container(border=True):
            ...
            try:
                res = requests.get("http://127.0.0.1:8000/api/settings/maintenance", headers=headers)
                current_mode = res.json().get("maintenance_mode", False) if res.status_code == 200 else False
            except:
                current_mode = False
            ...
            new_mode = st.checkbox(
                "Maintenance Mode",
                value=current_mode,
                help="Disable access for annotators and reviewers"
            )
            
            if new_mode != current_mode:
                try:
                    res = requests.post(
                        "http://127.0.0.1:8000/api/settings/maintenance",
                        json={"maintenance_mode": new_mode},
                        headers=headers
                    )
                    if res.status_code == 200:
                        st.success("Maintenance mode updated!")
                    else:
                        st.error("Failed to update maintenance mode")
                except Exception as e:
                    st.error(f"Error: {e}")
